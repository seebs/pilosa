...
func (sc *sliceContainers) Update(key uint64, fn func(c *Container, existed bool) (output *Container, write bool)) {
	i := search64(sc.keys, key)

	// If index is negative then there's not an exact match
	// and a container needs to be added.
	if i < 0 {
		c, write := fn(nil, false)
		if write {
			sc.insertAt(key, c, -i-1)
		}
	} else {
		c, write := fn(sc.containers[i], true)
		if write {
			sc.containers[i] = c
		}
	}
...
func (sc *sliceContainers) UpdateEvery(fn func(k uint64, c *Container) (update *Container, write bool, done bool, err error)) error {
	for i, k := range sc.keys {
		newC, write, done, err := fn(k, sc.containers[i])
		if write {
			sc.containers[i] = newC
		}
		if err != nil {
			return err
		}
...
func (sc *sliceContainers) PutContainerValues(key uint64, typ byte, n int, shared bool) {
	i := search64(sc.keys, key)
	if i < 0 {
		c := NewContainer()
		c.typ = typ
